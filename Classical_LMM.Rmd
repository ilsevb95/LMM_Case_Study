...
ggplot(data, aes(Time, Weight_change)) + 
  geom_point() + 
  ...
  xlab("Time") +
ylab("Weight Change") + 
ggtitle("Relation between Weight Change and Time" ) 
...
plot(mod_poly)# from the residual plot we can see that the variance is not constant,
...
ggplot(data,aes(data$Weight_change, col = time_try)) + 
  geom_histogram() + 
  xlab("Weight Change") + 
  ylab("Frequency") + facet_grid(~data$Time)

## HIST W/LOG 

ggplot(data,aes(log(data$Weight_change), col = time_try)) + 
  geom_histogram() + 
  xlab("Weight Change") + 
  ylab("Frequency") + facet_grid(~data$Time)
